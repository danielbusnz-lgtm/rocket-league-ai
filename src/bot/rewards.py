"""
Reward Functions for Rocket League Bot Training (RLGym 2.0)

Based on official RLGym 2.0 documentation.
"""
...
class TouchReward(RewardFunction[AgentID, GameState, float]):
    ...
    def get_rewards(self, agents: List[AgentID], state: GameState, is_terminated: Dict[AgentID, bool],
                    is_truncated: Dict[AgentID, bool], shared_info: Dict[str, Any]) -> Dict[AgentID, float]:
        return {agent: 1.0 if state.cars[agent].ball_touched else 0.0 for agent in agents}
